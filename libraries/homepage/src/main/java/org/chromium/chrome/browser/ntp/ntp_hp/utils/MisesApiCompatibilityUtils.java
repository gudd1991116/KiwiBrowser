...
public class MisesApiCompatibilityUtils {
    private MisesApiCompatibilityUtils() {
    }
        ...
        public void run() {
            ...
            if (!mActivity.isFinishing()) {
                if (mTryCount < MAX_TRY_COUNT) {
                    MisesThreadUtils.postOnUiThreadDelayed(this, RETRY_DELAY_MS);
                } else {
                    mActivity.finish();
                }
            }
    ...
    public static Uri getUriForImageCaptureFile(File file) {
        return Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2
                ? MisesContentUriUtils.getContentUriFromFile(file)
                : Uri.fromFile(file);
    }
    ...
    public static Uri getUriForDownloadedFile(File file) {
        return Build.VERSION.SDK_INT > Build.VERSION_CODES.M
                ? MisesFileUtils.getUriForFile(file)
                : Uri.fromFile(file);
    }
