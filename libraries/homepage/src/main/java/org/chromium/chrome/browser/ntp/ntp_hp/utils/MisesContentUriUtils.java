...
import org.chromium.chrome.browser.ntp.ntp_hp.base.MisesCalledByNative;
 ...
 */
public abstract class MisesContentUriUtils {
    ...
    public static String getMimeType(String uriString) {
        ContentResolver resolver = MisesContextUtils.getApplicationContext().getContentResolver();
    ...
    private static boolean isVirtualDocument(Uri uri) {
        ...
        if (!DocumentsContract.isDocumentUri(MisesContextUtils.getApplicationContext(), uri)) {
            return false;
        }
        ContentResolver contentResolver = MisesContextUtils.getApplicationContext().getContentResolver();
        try (Cursor cursor = contentResolver.query(uri, null, null, null, null)) {
            if (cursor != null && cursor.getCount() >= 1) {
                cursor.moveToFirst();
