...
/**
 * {@link <a href="https://github.com/shenbengit/PagerGridLayoutManager">...</a>}
 */

public class MisesPagerGridLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    private MisesPagerGridSnapHelper mPagerGridSnapHelper;
    /**
     * 当前滑动方向
     */
    @Orientation
    ...
    private float mMillisecondPreInch = MisesPagerGridSmoothScroller.MILLISECONDS_PER_INCH;
    private int mMaxScrollOnFlingDuration = MisesPagerGridSmoothScroller.MAX_SCROLL_ON_FLING_DURATION;
    ...
    public MisesPagerGridLayoutManager(@IntRange(from = 1) int rows, @IntRange(from = 1) int columns) {
        this(rows, columns, HORIZONTAL);
    }

    public MisesPagerGridLayoutManager(@IntRange(from = 1) int rows, @IntRange(from = 1) int columns, @Orientation int orientation) {
    ...
    public void onAttachedToWindow(RecyclerView view) {
        ...
        if (isInScrollingContainer(view)) {
            ...
            if (isHandlingSlidingConflictsEnabled) {
                onItemTouchListener = new MisesPagerGridItemTouchListener(this, view);
        ...
        mPagerGridSnapHelper = new MisesPagerGridSnapHelper();
...
//        mPagerChangedListener = null;
     ...
     * 默认值：{@link MisesPagerGridSmoothScroller#MILLISECONDS_PER_INCH}
     * <p>
     * set millisecond pre inch. not too small.
     * default value: {@link MisesPagerGridSmoothScroller#MILLISECONDS_PER_INCH}
     *
     * @param millisecondPreInch 值越大，滚动速率越慢，反之
     * @see MisesPagerGridSmoothScroller#calculateSpeedPerPixel(DisplayMetrics)
     */
     ...
     * 默认值：{@link MisesPagerGridSmoothScroller#MAX_SCROLL_ON_FLING_DURATION}，单位：毫秒
     * <p>
     * set max scroll on fling duration.If you want this value to expire, use {@link Integer#MAX_VALUE}
     * default value: {@link MisesPagerGridSmoothScroller#MAX_SCROLL_ON_FLING_DURATION},Unit: ms
     *
     * @param maxScrollOnFlingDuration 值越大，滑动时间越长，滚动速率越慢，反之
     * @see MisesPagerGridSmoothScroller#calculateTimeForScrolling(int)
     */
    ...
    public void smoothScrollToPagerIndex(@IntRange(from = 0) int pagerIndex) {
        ...
        if (Math.abs(pagerIndex - previousIndex) > 3) {
            ...
            MisesPagerGridSmoothScroller smoothScroller = new MisesPagerGridSmoothScroller(mRecyclerView, this);
...
//                removeAndRecycleView(childAt, recycler);
    ...
    private static class SmoothScrollToPosition implements Runnable {
        ...
        @NonNull
        private final MisesPagerGridLayoutManager mLayoutManager;
        @NonNull
        private final RecyclerView mRecyclerView;

        SmoothScrollToPosition(int position, @NonNull MisesPagerGridLayoutManager layoutManager, @NonNull RecyclerView recyclerView) {
            mPosition = position;
        ...
        @Override
        public void run() {
            MisesPagerGridSmoothScroller smoothScroller = new MisesPagerGridSmoothScroller(mRecyclerView, mLayoutManager);
